...
// Making function static as it is only used in the scope of this source file
static void populate_texture_stimulus(SDL_Renderer *renderer, int x, int y,
...
static void destroy_context(SDL_Window *window, SDL_Renderer *renderer,
  ...
  if (outlet != NULL) {
    ...
    printf("Destroying LSL outlet\n");
    lsl_destroy_outlet(outlet);
  }
  if (inlet != NULL) {
    printf("Destroying LSL inlet\n");
...
int run_paradigm() {
  ...
  int i = 0;
  while (paradigm_stop_event == 0) {
    // Check inlet if relevant marker present
    lsl_pull_sample_str(inlet, &curr_marker, 1, 0, &errcode);
